This authentication provider was forked from [authts/react-oidc-context](https://github.com/authts/react-oidc-context). At the time of forking, the original repo was not considered as stable enough to be included as a dependency to the project.

Since it's sources only consist of the few files present in this folder (and sub-folders), we considered (manually) forking a good alternative for security matters.

If you want to take a look at the original repo, here was the commit the (manual) fork refers to :

- [Merge pull request #1345](https://github.com/authts/react-oidc-context/commit/a779b2e0162738589f3bbadb6f764642a33d7241)
  - Commit from the `main` branch after [release v3.1.0](https://github.com/authts/react-oidc-context/releases/tag/v3.1.0)

From there, this fork will evolve on it's own, especially for particular Keycloak integrations we might integrate into this react template. To track local changes, use your prefered git-diff tool and analyse the file history. (An initial commit was dedicated to the (manual) forking process)

However, if you want to revert to using the original repo as a dependency, do the following :

- remove the `contexts/auth` folder from your sources.
- run `npm install react-oidc-context --save`

For reference, below is the original README file content from the source repository (at [a779b2e](https://github.com/authts/react-oidc-context/commit/a779b2e0162738589f3bbadb6f764642a33d7241)) :
...
- [react-oidc-context](#react-oidc-context)
  ...
  - [Getting Started](#getting-started)
    ...
    - [Automatic sign-in](#automatic-sign-in)
