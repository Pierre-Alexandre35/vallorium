  ...
  React.useEffect(() => {
    ...
    void (async (): Promise<void> => {
      ...
      try {
        ...
        /**
         * removed this as it magically fixed perf issues and weird behaviors on some HUMS related sessions problems.
         * TODO : figure out why
         */
        user = !user ? await userManager.getUser() : user;

        dispatch({ type: 'INITIALISED', user: user || null });
      } catch (error) {
        if (error.message === 'No matching state found in storage') {
          await userManagerContext.signoutRedirect();
        } else {
